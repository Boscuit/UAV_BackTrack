...
Initializer::Initializer(const vector<cv::KeyPoint> &ReferenceKeyPoints, cv::Mat K, float sigma, int iterations)
{
    mK = K;

    mvKeys1 = ReferenceKeyPoints;
...
bool Initializer::Initialize(const vector<cv::KeyPoint> &CurrentKeyPoints, const vector<int> &vMatches12, cv::Mat &R21, cv::Mat &t21,
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    // Fill structures with current keypoints and matches with reference frame
    // Reference Frame: 1, Current Frame: 2
